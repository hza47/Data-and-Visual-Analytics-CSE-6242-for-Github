...
load_ts <- function(csv_filename) {
    ...
    #csv_filename <- "data/realAWSCloudwatch/ec2_cpu_utilization_5f5533.csv"
    df <- read.csv(csv_filename, stringsAsFactors=FALSE)
    
    # TODO: convert timestamp column to POSIX datetime
    orderBy <- as.POSIXct(df$timestamp, format = "%Y-%m-%d %H:%M:%S")
    
    # TODO: create xts time series from dataframe
    s <- xts( x = df$value, order.by = orderBy)
    
    return(s)  # return time series
}

find_anomalies <- function(s, window_size, threshold) {
    ...
    s.peaks <- (s.mean + (threshold * s.sd))
    s.troughs <- (s.mean - (threshold * s.sd))
    peaksAnomaliesIndicies <- index(s)[which(s >= s.peaks)]
    troughsAnomaliesIndicies <- index(s)[which(s <= s.troughs)]
    anomoliesIndicies <- c(peaksAnomaliesIndicies, troughsAnomaliesIndicies)
    anomalies <- s[anomoliesIndicies]
...
visualize <- function(res, wins=NA, title="Anomaly Detection Results") {
  ...
  plot_2 <- plot_1  + geom_line(aes(y=s, x=index(s))) + geom_line(aes(y=(res$s.mean), x=index(s), colour = 'green'))

  # TODO: Draw margins at mean +/- (threshold * s.d.)
  plot_3 <- plot_2 + geom_line(aes(y=(res$s.mean - (res$threshold * res$s.sd)), x=index(s), colour = 'red')) + geom_line(aes(y=(res$s.mean + (res$threshold * res$s.sd)), x=index(s), colour = 'blue')) + labs(x="Time", y="Value") + ggtitle("Plot of Time vs Value")
  plot_3
  
  # TODO: Mark anomalies
  lines(res$s.mean,col="green")
  lines((res$s.mean + (res$threshold * res$s.sd)), col="blue")
  lines((res$s.mean - (res$threshold * res$s.sd)), col="blue")
